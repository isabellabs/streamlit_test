...
st.title(":material/database: Dataset name :primary-badge[Waiting verification]")
tab1, tab2, tab3, tab4 = st.tabs(["Understand the generation", "Analyse metrics", "Inspect synthetic data", "Sumarize feedback"])

with tab1:
    # -- Dataset information --
    Col1, Col2 = st.columns([2, 1])
    with Col1:
        st.badge("Step 1 of 4", color="gray")
        st.write("Get to know the generation base: Generation purpose, Medical data used, Generative model used.")
        st.warning('**Consider that** limitations in the medical dataset used for generation may be reflected in the generated data, resulting in underrepresentation, bias and shortcomings cases.', icon=":material/warning:", width="stretch")
        ...
        with st.expander ("Real dataset information", expanded=True):
            ...
            st.markdown(real_dataset_info, unsafe_allow_html=True)

        # -- Generative model information --
        with st.expander ("Generative model information", expanded=True):
            ...
            st.markdown(generative_model_info, unsafe_allow_html=True)
    ...
    with Col2:
        with st.form("representativeness_form"):
            ...
            representativeness_level = st.radio(
...
with tab2:
    st.badge("Step 2 of 4", color="gray")

with tab3:
    st.badge("Step 3 of 4", color="gray")
